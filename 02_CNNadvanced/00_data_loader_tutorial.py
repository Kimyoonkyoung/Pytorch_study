...
'''
csv 에 이미지 이름이랑 annotation 적어둔거 읽어오는 예제
꼭 csv 가 아니더라도 pytorch 는 그냥 .as_matrix().astype('float') 로만 받아오면 되는 것 같다
'''
######################################################################
...
'''
Dataset 이라는 클래스(torch.utils.data.Dataset) 는 abstract class 이다.
따라서 내가 override 해주어야 한다.
__len__, __getitem__, __init__
'''
######################################################################
# Dataset class
# -------------
...
class FaceLandmarksDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        landmarks = self.landmarks_frame.ix[idx, 1:].as_matrix().astype('float') # 첫번째 column 빼고 뒷부분 모두 다 모아서 float 으로 쭈르륵 받아온다
        landmarks = landmarks.reshape(-1, 2)
        sample = {'image': image, 'landmarks': landmarks} # dictionary 로 관리
...
#
'''
isinstance 함수는 해당 값의 자료형 형태를 확일할 떄 쓰는 함수 이다 (클래스의 instance인지도 확인 가능)
즉 리턴은 True, False
isinstance(number, int)  --> 만약 number라는 변수가 int 형이면 True, 아니면 False
isinstance(c, Dog)  --> 만약 c 라는 object 가 Dog 의 instance 이면 True, 아니면 False
'''

'''
Rescale 에서 output_size 를
int 로 넣어주면 더 작은쪽을 그 크기로 맞추고 비율 유지
tuple 로 넣어주면 그대로 맞춤. tuple 로 넣어준다는 것은 [desired height, desired width] 로 넣어주는걸 의미하는듯함
'''
...
'''
torch.utils.data.DataLoader 클래스는
batch, shuffle, parallel load 를 모두 가능하게 해준다

오 내가 위에서 공부하며 내려온 customizing 한 함수와 클래스를 DataLoader 첫번째 parameter로 넣어주기만 하면된다
짱편하다!!!
'''
######################################################################
...
for i_batch, sample_batched in enumerate(dataloader):
    ...
    if i_batch == 3:
        ...
        '''
        위에서 했던 customizing 함수들을 굳이 안쓰고 사용할 수 있는 것이 torchvision 이 있다
        torchvision 의 ImageFolder 라는 클래스를 사용하면 위의 일련의 과정들이 다 구현되어 있다
        하지만 ImageFolder를 사용하기 위해서는 특별히 데이터 구조를 맞춰주어야 한다

        'root/클래스라벨/이미지'

        저렇게 맞춰줄려면.. 풀고자하는 문제가 무조건 classification 이어야겠군
        '''
        ######################################################################
